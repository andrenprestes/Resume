...
export default function EventoScreen({navigation, route}) {
    const [pressed, setPressed] = useState(false);
    const [lista, setLista] = useState();

    const run = async () => {
        await userApi.verifyContent(route.params.user,"Evento",route.params.id).then(response => {
    ...
    return (
        <>
            ...
            <View style={styles.container}>
                ...
                <Text>Evento user: {route.params.user}, id: {route.params.id}</Text>
                {pressed
